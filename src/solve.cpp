...
dfs_check_solved(DfsArg *arg)
{
	...
	{
		if ((arg->step->is_valid(arg->current_alg) || arg->opts->all) && (!arg->step->final || !cancel_niss(arg)))
		{
...
instance_thread(void *arg)
{
	...
	while (1)
	{
		...
		c = node->alg->inv[0] ? apply_move(node->alg->move[0], inverse_cube(td->cube)) : apply_move(node->alg->move[0], td->cube);
...
invert_branch(DfsArg *arg)
{
	...
	aux = arg->cube;
	arg->cube = is_solved(arg->inverse) ? inverse_cube(arg->cube) : arg->inverse;
	arg->inverse = aux;
