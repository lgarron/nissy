...
int findchunk(PruneData *pd, int nchunks, uint64_t i)
{
	uint64_t chunksize;
...
void genptable(PruneData *pd, int nthreads)
{
	...
	fprintf(stderr, "Depth %d done, generated %" PRIu64 "\t(%" PRIu64 "/%" PRIu64 ")\n",
...
instance_bfs(void *arg)
{
	...
	rmax = td->thid == td->nthreads - 1 ? td->pd->coord->max : ((uint64_t)td->thid + 1) * blocksize;
